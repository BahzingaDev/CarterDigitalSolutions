...
export function AdminCustomers({ csrfToken, onOpenEnquiry, onOpenProject, refreshKey = 0 }: {
  csrfToken: string;
  ...
  return <div className="admin-workspace-split">
    ...
    {draft ? <section className="admin-panel admin-workspace-editor">
      ...
      <button className="btn btn-accent" onClick={() => void save()} type="button"><Save size={16} /> Save profile</button>
      <section className="admin-customer-history">
        <h3>Enquiries</h3>
        {draft.enquiries.map((item) => <button key={item.id} onClick={() => onOpenEnquiry(item.id, item.type === 'quote')} type="button"><span><strong>{item.project_type || item.type}</strong><small>{item.status} · {formatCurrency(item.estimated_cost)}</small></span><ArrowRight size={16} /></button>)}

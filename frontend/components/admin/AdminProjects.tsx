...
export function AdminProjects({ csrfToken, initialProjectId, initialTab, onInvoiceSent, onTabChange, refreshKey = 0 }: { csrfToken: string; initialProjectId?: string | null; initialTab?: WorkspaceTab; onInvoiceSent?: () => Promise<void>; onTabChange?: (tab: WorkspaceTab) => void; refreshKey?: number }) {
  ...
  useEffect(() => {
    void fetchAdminProjects()
      .then((projects) => {
        ...
        if (target) { const targetTags = target.tags.join(', '); setDraft(target); setTags(targetTags); setSavedFingerprint(projectFingerprint(target, targetTags)); setEditing(true); setTab(initialTab ?? (target.invoices.length ? 'invoices' : 'delivery')); }
      })
...
function normaliseProject(project: AdminProject): AdminProject {
  return {
    ...
    included_consultation_hours: project.included_consultation_hours ?? 8,

...
export function AdminCommunicationSettings({ csrfToken }: { csrfToken: string }) {
  ...
  return <section className="admin-panel admin-workspace-editor admin-signature-panel"><div className="admin-panel-heading"><div><h2>Email signature</h2><p>Appended to messages sent from the enquiry composer.</p></div></div>{error ? <div className="alert alert-danger" role="alert">{error}</div> : null}<div className="admin-template-field"><AdminRichTextEditor label="Signature" maxLength={2000} onChange={(value) => { setSignature(value); setSaved(false); }} placeholders={enquiryPlaceholders} value={signature} /><PlaceholderSelect definitions={enquiryPlaceholders} label="Insert signature placeholder" onInsert={(key) => { setSignature((current) => `${current}{{${key}}}`); setSaved(false); }} /></div><PlaceholderReference definitions={enquiryPlaceholders} title="Signature placeholder reference" /><button className="btn btn-accent" disabled={isSaving || saved} onClick={() => void save()} type="button"><Save size={16} /> {isSaving ? 'Saving...' : saved ? 'Saved' : 'Save signature'}</button></section>;
}

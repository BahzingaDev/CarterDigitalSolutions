...
export function AdminTemplates({ csrfToken, onDirtyChange }: { csrfToken: string; onDirtyChange?: (isDirty: boolean) => void }) {
  ...
  return <div className="admin-workspace-split">
    <section className="admin-panel admin-workspace-list"><button className="btn btn-accent" onClick={() => select(empty)} type="button"><Plus size={16} /> New template</button>{visibleItems.map((item) => <button className={draft.id === item.id ? 'is-active' : ''} key={item.id} onClick={() => select(item)} type="button"><strong>{item.name}</strong><small>{item.subject}</small></button>)}{items.length === 0 ? <p className="admin-empty">No templates yet.</p> : null}<AdminPagination count={items.length} onPageChange={setPage} page={page} /></section>
...
function TemplateField({ label, multiline, onChange, value }: { label: string; multiline: boolean; onChange: (value: string) => void; value: string }) { return multiline ? <AdminRichTextEditor label={label} onChange={onChange} placeholders={enquiryPlaceholders} value={value} /> : <PlaceholderInput definitions={enquiryPlaceholders} label={label} onChange={onChange} value={value} />; }

  ...
  return (
    <nav className="site-navbar navbar navbar-expand-lg border-bottom">
      <div className="container">
        ...
        <button className="navbar-toggler" type="button" aria-controls="primaryNavigation" aria-expanded={isNavOpen} aria-label="Toggle navigation" onClick={() => { setIsNavOpen((current) => !current); setOpenMenu(null); }}>
        ...
        <div className={`collapse navbar-collapse ${isNavOpen ? 'show' : ''}`} id="primaryNavigation">
          <ul className="navbar-nav nav-primary mb-3 mb-lg-0">
            {standardLinks.map((link) => <li className="nav-item" key={link.label}><a className="nav-link" href={link.href} onClick={closeNavigation}>{link.label}</a></li>)}
